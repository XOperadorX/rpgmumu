...
function criarDivClasse(classe, left, top, width=36, height=36){
    ...
    el.style.transition = 'left 0.3s, top 0.3s'; // animação suave
    return el;
}
...
function desenharInimigos(lista){
    lista.forEach(en=>{
        if(!en._el){
            en._el = criarDivClasse('charIcon enemy', en.Xpos*tileSize+tileSize/2, en.Ypos*tileSize+tileSize/2);
            en._el.title = `${en.Name} (Lv ${en.Level})\nHP:${en.HP}/${en.MaxHP}\nElemento:${en.Element}`;
            mapWrap.appendChild(en._el);
        } else {
            en._el.style.left = (en.Xpos*tileSize+tileSize/2)+'px';
            en._el.style.top  = (en.Ypos*tileSize+tileSize/2)+'px';
        }
    });
}

// ======= Desenha outros jogadores =======
function desenharOutrosJogadores(lista){
    lista.forEach(ch=>{
        if(ch.CharID === charID) return;
        if(!ch._el){
            ch._el = criarDivClasse('charIcon other', ch.Xpos*tileSize+tileSize/2, ch.Ypos*tileSize+tileSize/2);
            ch._el.style.background = '#' + ((ch.PlayerID*99999)&0xFFFFFF).toString(16).padStart(6,'0');
            ch._el.title = `${ch.Name} (${ch.Class})\nLv ${ch.Level}\nHP:${ch.HP}\nMana:${ch.Mana}`;
            mapWrap.appendChild(ch._el);
        } else {
            ch._el.style.left = (ch.Xpos*tileSize+tileSize/2)+'px';
            ch._el.style.top  = (ch.Ypos*tileSize+tileSize/2)+'px';
        }
    });
}
...
function moverInimigos(){
    const occupied = enemies.map(e=>`${e.Xpos},${e.Ypos}`);
    enemies.forEach(en=>{
        let nx = en.Xpos;
        let ny = en.Ypos;

        // distância para jogador
        const dx = playerX - en.Xpos;
        const dy = playerY - en.Ypos;
        let moveMade = false;
        ...
        if(Math.abs(dx)<=3 && Math.abs(dy)<=3){
            ...
            if(!tryMove(nx,ny) || occupied.includes(`${nx},${ny}`)){
                nx = en.Xpos;
                ny = en.Ypos;
            } else {
                moveMade = true;
        ...
        // se não perseguiu, mover aleatório
        if(!moveMade){
            const dirs = [[0,1],[0,-1],[1,0],[-1,0]];
            const [ddx,ddy] = dirs[Math.floor(Math.random()*dirs.length)];
            const tx = en.Xpos+ddx;
            const ty = en.Ypos+ddy;
            if(tryMove(tx,ty) && !occupied.includes(`${tx},${ty}`)){
                nx = tx; ny = ty;
            }
        }

        // atualizar posição e evitar sobreposição
        en.Xpos = nx; en.Ypos = ny;
        if(nx===playerX && ny===playerY){ iniciarBatalha(en); }
        occupied.push(`${nx},${ny}`);
    });
...
// ======= Loop inimigos =======
setInterval(moverInimigos, 1500);
