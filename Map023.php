...
// ====== Endpoint AJAX: atualização de posição ======
...
$q = sqlsrv_query($conn, "
    SELECT TOP 1 CharID, Name, Class, Level, HP, MaxHP, Mana, MaxMana, Power, Exp
    FROM dbo.Characters
    WHERE PlayerID = ?
", [$playerID]);
...
$enemies=[];
$sqlEnemies = "
SELECT e.EnemyID,e.Name, ISNULL(e.HP,0) AS HP, ISNULL(e.MaxHP,0) AS MaxHP, ISNULL(e.Level,1) AS Level,
...
function sendPosition(x,y){
    ...
    data.append('action','move'); data.append('x',x); data.append('y',y); data.append('charid',charID);
