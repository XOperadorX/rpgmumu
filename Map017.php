...
<div class="map-wrap" id="mapWrap">
  <div class="grid" id="grid">
    ...
    // Exemplo de array de inimigos
    $enemies = [
        ['X' => 10, 'Y' => 10, 'Name' => 'Goblin'],
        ['X' => 20, 'Y' => 20, 'Name' => 'Orc']
    ];

    // Desenha inimigos como SVGs
    foreach ($enemies as $en) {
        echo "<img class='char enemy' data-x='{$en['X']}' data-y='{$en['Y']}' title='{$en['Name']}' 
        src=\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='36' height='36'>
            <circle cx='18' cy='12' r='8' fill='%23ff4444'/>
            <rect x='8' y='20' width='20' height='12' rx='3' ry='3' fill='%23aa0000'/>
        </svg>\">";
    }
    ?>
  </div>
...
function atualizarInimigosVisual(enemiesData) {
    document.querySelectorAll('.charIcon.enemy, .enemyLabel').forEach(el => el.remove());
    enemies.length = 0;
    enemiesData.forEach(en => enemies.push(en));
    desenharInimigos(enemiesData);
}

function atualizarOutrosVisual(outros) {
    document.querySelectorAll('.charIcon.other').forEach(el => el.remove());
    desenharOutrosJogadores(outros);
}

function syncEmTempoReal() {
    fetch('sync.php')
        .then(r => r.json())
        .then(data => {
            if(data.error) return;
            atualizarHUD(data.char);
            atualizarInimigosVisual(data.enemies);
            atualizarOutrosVisual(data.others);
        })
        .catch(() => {});
}
...
setInterval(syncEmTempoReal, 3000);
</script>
